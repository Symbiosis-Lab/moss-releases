...
`title:` 有两个作用。一是作为页面的*界面标签*——用于浏览器标签栏、导航链接、面包屑、RSS 和社交媒体元数据；二是在**文章**页面正文中没有 `# H1`（且 `:::hero` 块中也没有 H1）时，moss 会在正文顶部注入 `<h1 class="moss-article-title">{title}</h1>`，让读者始终能看到一个可见的页面标题。若没有 `title:`，moss 会用文件名（首字母大写）作为兜底，两个用途同步——这符合 Obsidian 中"文件名即为文档标题"的惯例。

如果想在某篇文章上单独关闭自动注入的标题（比如某个像首页一样精心排版的特殊页面），可以写 `title: ""`（显式的空字符串）。这与不写 `title:` 不同——不写会回退到文件名。文件夹/索引页面（`index.md`、`main.md`、自命名的 `foo/foo.md` 等）从不注入；它们按作者所写的正文渲染。
