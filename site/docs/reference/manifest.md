...
| `requires` | `string[]` | no | Privileged host capabilities this plugin needs granted. Recognized values: `execute_binary` (run a native program), `identity_sign` (sign with the site identity key and read its public key). A call to one of these fails unless it is declared here. |
