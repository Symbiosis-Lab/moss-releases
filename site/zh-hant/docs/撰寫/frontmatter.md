...
由於標題會回退到檔名，你可以用頁面自己的語言命名檔案，讓 moss 替你產生標題——`隱私.md` 渲染為「隱私」，`Privacy.md` 渲染為「Privacy」，不需要 `title:` 欄位，也不必在正文寫 `# H1`。當檔名不是 ASCII 時，用 `url` 欄位釘住一個簡短穩定的網址，讓發佈路徑保持乾淨：`url: privacy` 讓 `隱私.md` 發佈在 `/privacy`。
